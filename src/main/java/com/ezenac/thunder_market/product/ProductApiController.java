...
public class ProductApiController {
    ...
    public ResponseEntity<byte[]> getImage(String imageURL) {
        try {
            ...
            // 이미지 파일
            File file = imageService.getImage(filePath);
    ...
    public ResponseEntity<ProductImageDTO> registerImage(@PathVariable Long productId, MultipartFile file) {
        ...
        ProductImageDTO productImageDTO = imageService.addImage(productId, file);
        log.info("addImage =>" + productImageDTO);
    ...
    @PreAuthorize("isAuthenticated()")
    @PutMapping("/images")
    public ResponseEntity<ProductImageDTO> changeImage(Long imageId, MultipartFile file) {
        if (!file.isEmpty() || imageId == null) { //파일 유효성 검사

            ProductImageDTO productImageDTO = imageService.changeImage(imageId, file);
